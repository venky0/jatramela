    ...
    <div style={{ background: "var(--bg-primary)" }}>
      ...
      <div className="content-container py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {CATEGORIES.map(category => (
            <div key={category.id} className="heritage-card p-6 flex flex-col justify-between">
              <div>
                <h2 className="text-3xl font-extrabold mb-3 text-gradient-red" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                  {category.name}
                </h2>
                ...
                <div className="space-y-4">
                  {category.tools.map(tool => (
                    <Link key={tool.href} href={tool.href} className="block group">
                      <div className="p-3.5 rounded-xl border transition-all duration-200 hover:border-[var(--text-gold)] hover:bg-[var(--bg-card-hover)]"
                        style={{ background: "var(--bg-secondary)", borderColor: "var(--border)" }}>
                        <p className="font-extrabold text-base text-[var(--text-primary)] group-hover:text-[var(--text-gold)] transition-colors" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                          {tool.name} →
                        </p>
                        <p className="text-sm mt-1.5 leading-normal" style={{ color: "var(--text-subtle)" }}>
                          {tool.desc}
                        </p>
                      </div>
                    </Link>
              ...
              <div className="pt-6 mt-6 border-t" style={{ borderColor: "var(--border)" }}>
                <Link href={category.href}>
                  <button className="w-full btn-gold py-2.5 text-base">
                    View Category Page
                  </button>
                </Link>
              </div>
        ...
        <div className="mt-12 p-8 rounded-2xl border text-center max-w-2xl mx-auto" style={{ background: "var(--bg-section-alt)", borderColor: "var(--border)" }}>
          ...
          <h3 className="text-2xl font-extrabold mb-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>100% In-Browser Privacy Guard</h3>

    ...
    <div style={{ background: "var(--bg-primary)" }}>
      ...
      <div className="content-container py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {CATEGORIES.map(category => (
            <div key={category.id} className="heritage-card p-6 flex flex-col justify-between">
              <div>
                <h2 className="text-xl font-extrabold mb-3 text-gradient-red" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                  {category.name}
                </h2>
                ...
                <div className="space-y-4">
                  {category.tools.map(tool => (
                    <Link key={tool.href} href={tool.href} className="block group">
                      <div className="p-3 rounded-xl border transition-all duration-200 hover:border-[#C9A84C] hover:bg-white/5"
                        style={{ background: "var(--bg-secondary)", borderColor: "var(--border)" }}>
                        <p className="font-extrabold text-xs group-hover:text-[#C9A84C] transition-colors" style={{ fontFamily: "'Baloo 2', sans-serif" }}>
                          {tool.name} →
                        </p>
                        <p className="text-[10px] mt-1 leading-normal" style={{ color: "var(--text-subtle)" }}>
                          {tool.desc}
                        </p>
                      </div>
                    </Link>
        ...
        <div className="mt-12 p-8 rounded-2xl border text-center max-w-2xl mx-auto" style={{ background: "var(--bg-section-alt)", borderColor: "var(--border)" }}>
          ...
          <h3 className="text-lg font-extrabold mb-2" style={{ fontFamily: "'Baloo 2', sans-serif" }}>100% In-Browser Privacy Guard</h3>
